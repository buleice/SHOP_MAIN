 ...
 * @LastEditTime: 2019-04-09 14:33:48
 -->
<template>
    <div>
      <ul class="top">
      ...
      <ul class="middle">
        <li class="draw_status" v-show="cashing>0"><span>{{cashing}}元 </span>正在提现</li>
        <li class="detail_info"> <router-link to="/bonus/cash/record">提现详情 ></router-link> </li>
      </ul>
      <div class="banke_card" v-if="cardinfo.Fbank">
        ...
        <div class="card_edit" @click="$router.push({path:'/bonus/cash/bindcard', query:{action:'edit'}})">修改信息</div>
      </div>
      <div @click="$router.push({path:'/bonus/cash/bindcard'})" class="banke_card" v-if="!cardinfo.Fbank">
...
import {GET_NONUS_CANSH_INFO} from '../../api/pageDataApis'
    export default {
        name: "myBonusCanDraw",
        data(){
          return{
        ...
        created(){
          this.$nextTick(function(){
            GET_NONUS_CANSH_INFO().then(Response=>{
        ...
        methods:{
          gotoRightPage(){
            if (this.cashing>0) {
              ...
              this.$router.push({name:'userCash'})
              :
              this.$router.push({name:'BindBankcard'})
            }
          },
          ...mapActions('moduleBinkCard',['setCanCash','setCardInfo','setCashing']),
        },
        computed:{
          lastFour(){
            return this.cardinfo.FbankcardNo.substr(-4,4)
          },
          ...mapGetters('moduleBinkCard',["cancash","cardinfo","cashing"])
        }
    }
</script>
...
.card_edit{height: 2.25rem;color:#a3a3a3; line-height: 2.25rem;margin-top: -5.3rem;text-align: right;}
...
.rule>ul{padding:0 .63rem;text-align: left;}

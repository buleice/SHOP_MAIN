 ...
 * @LastEditTime: 2019-04-09 14:32:49
 -->
<template lang="html">
  <div class="bindCard">
    <div class="weui-cells">
      <div class="weui-cell">
        ...
        <div class="weui-cell__bd ">
          <input class="weui-input" v-model="bankCardInfo.Fname" type="text" placeholder="">
        </div>
      </div>
    </div>
    <div class="weui-cells weui-cells_form">
      <div class="weui-cell">
        ...
        <div class="weui-cell__bd ">
          <input class="weui-input" v-model="bankCardInfo.FidcardNo" type="number" placeholder="">
        </div>
      </div>
    </div>
    <div class="weui-cells weui-cells_form">
      <div class="weui-cell">
        ...
        <div class="weui-cell__bd ">
          <input class="weui-input" v-model="bankCardInfo.FbankcardNo" type="number" placeholder=""
            @focus="BankNameDiable=true" @change="_checkBank">
        </div>
      </div>
    </div>
    <div v-if='!BankNameDiable' class="weui-cell weui-cell_select weui-cell_select-after"
      style="background: #ffffff;font-size:17px">
      <div class="weui-cell__hd">
    ...
    <div v-else class="weui-cells weui-cells_form">
      <div class="weui-cell">
        <div class="weui-cell__hd"><label class="weui-label">开户银行</label></div>
        <div class="weui-cell__bd">
          <input class="weui-input" type="text" :value="BankName" disabled>
        </div>
      </div>
    </div>
    <div class="weui-cells weui-cells_form" @click="showAreaList=true">
      <div class="weui-cell">
        <div class="weui-cell__hd"><label class="weui-label">开户地区</label></div>
        <div class="weui-cell__bd">
    ...
    <div class="weui-cells weui-cells_form">
      <div class="weui-cell">
        ...
        <div class="weui-cell__bd ">
          <input class="weui-input" v-model="bankCardInfo.Fphone" type="tel" placeholder="">
        </div>
        <button class="checkCode" :disabled="countdown<60" @click="changeCheckContent">
          {{checkContent}}
        </button>
      </div>
    </div>
    <div class="weui-cells weui-cells_form" v-if="showCheck">
      <div class="weui-cell">
        <div class="weui-cell__hd">
          <label class="weui-label">输入验证码</label>
        </div>
        <div class="weui-cell__bd ">
          <input class="weui-input" v-model="checkCode" type="number" placeholder="请输入短信验证码">
        </div>
      </div>
    </div>
...
<script>
  ...
  import {
    BIND_BANK_CARD,GENERATE_VERIFICATION_CODE
  ...
  export default {
    ...
    created() {
      if (JSON.stringify(this.cardinfo) !== '{}') {
        ...
        this.addressName=[{name:this.cardinfo.FbankProvince},{name:this.cardinfo.FbankCity},{name:this.cardinfo.FbankDistrict}]
        this.location = this.cardinfo.FbankProvince + ' ' + this.cardinfo.FbankCity + ' ' + this.cardinfo.FbankDistrict;
      }
    },
    methods: {
      _deepClone(obj) {
      ...
      submitCardInfo() {
        if (!this.testCardholder(this.bankCardInfo.Fname)) {
          this.alertContent = "请输入真实的持卡人姓名"
          this.isAlert = true;
          return false;
        } else if (this._chechCHNCardId(this.bankCardInfo.FidcardNo) == false) {
          this.alertContent = "身份证号不符合规则,请输入正确的身份证号"
          this.isAlert = true;
          return false;
        } else if (this._bankCardAttribution(this.bankCardInfo.FbankcardNo) == "error") {
          ...
          this.isAlert = true;
          return false;
        } else if (!this.testPhoneNumber(this.bankCardInfo.Fphone)) {
          ...
          return false;
           ...
           BIND_BANK_CARD(Object.assign({}, this.bankCardInfo, {
            FbankProvince: this.addressName[0].name,
            FbankCity: this.addressName[1].name,
            FbankDistrict: this.addressName[2].name,
            Fbank: this.BankName,
            code: this.checkCode,
      ...
      changeCheckContent() {
        if (this.testPhoneNumber(this.bankCardInfo.Fphone)) {
          this.showCheck = true
          this._settime()
          this.sendCode();
        } else {
      ...
      _settime() {
        let _this = this
        if (this.countdown == 0) {
          this.checkContent = "获取验证码";
          this.countdown = 60;
          return;
        } else {
          this.checkContent = this.countdown + 's' + '后重发';
          this.countdown--;
        }
        setTimeout(function () {
          _this._settime()
      ...
      sendCode() {
        GENERATE_VERIFICATION_CODE( {
          phone: this.bankCardInfo.Fphone
        })
      },
      pickerConfirm(res) {
      ...
      ...mapActions('moduleBinkCard', ['setCardInfo']),

    },
    computed: {
      ...mapGetters('moduleBinkCard', ['cardinfo'])
    },
  }
</script>

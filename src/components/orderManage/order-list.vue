 ...
 * @LastEditTime: 2019-04-09 14:19:06
 -->
<template>
  <div>
    <ul class="orders">
      <li class="order" v-for="(item,index) in orderData" :key="index">
        ...
        <ul class="data"
            @click="item.address_filled==0? (showAlertDialog=true,clickId=item.bill_id):$router.push({path:'/order/detail',query:{id:item.bill_id,goodsid:item.goods_id}})">
...
<script>
  ...
  import {GET_ORDER_LIST} from '../../api/pageDataApis'
  ...
  export default {
    ...
    methods: {
      initPageData() {
        GET_ORDER_LIST('/order/list.json').then(res => {
          this.orderData = res.data.list;
        });
      },
      delAok(){
        this.showAlertDialog=false;
        this.$router.push({path:'/order/place',query:{id:this.clickId}})
      },
